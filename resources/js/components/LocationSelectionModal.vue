...
const selectedCurrency = computed(
    () => page.props.selectedCurrency as Currency | null,
);
...
onMounted(async () => {
    try {
        ...
        if (currentLoc?.municipality?.id)
            form.municipality = currentLoc.municipality.id;
    } catch (error) {
...
function onCurrencyChange(event: Event) {
    ...
    router.post(
        setCurrency(),
        { currency_id: parseInt(target.value) },
        {
            preserveScroll: true,
        },
    );
        ...
        >
            <div class="flex items-center justify-between bg-primary px-6 py-4">
                <div>
                    ...
                    <p class="text-xs text-blue-100">
                        Para mostrarte los productos disponibles
                    </p>
                </div>
                <button
                    v-if="!isForced"
                    @click="$emit('close')"
                    class="rounded-full bg-secondary p-1 text-secondary-foreground hover:bg-secondary/80"
                >
                    <svg
                        class="size-5"
                        fill="none"
            ...
            <form @submit.prevent="submit" class="flex flex-col gap-4 p-6">
                <div v-if="loading" class="flex justify-center py-8">
                    <div
                        class="size-8 animate-spin rounded-full border-4 border-slate-200 border-t-blue-600"
                    ></div>
                </div>
